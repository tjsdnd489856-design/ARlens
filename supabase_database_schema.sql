-- =======================================================
-- ARlens B2B 데이터 플랫폼 확장을 위한 SQL 스키마 가이드 v3
-- Supabase SQL Editor에서 실행하세요.
-- =======================================================

-- 1. B2B 고객사(브랜드) 정보를 관리하는 브랜드 테이블 신설
...
-- 2. 사용자 인구통계 및 B2B 소속 정보를 관리하는 프로필 테이블 (확장)
...
-- 3. 딥 트래킹(Deep Tracking)을 위한 행동 로그 테이블 (확장)
...
CREATE TABLE IF NOT EXISTS public.activity_logs (
  ...
  brand_id text references public.brands(id) on delete set null, -- 해당 시점의 활성화된 브랜드 테마
  action_type text not null,                             -- 'select', 'capture', 'long_press', 'share' 등
  duration_ms int8 default 0,                            -- 렌즈 착용/상세보기 유지 시간 (밀리초)
  created_at timestamptz default now()
);

-- 4. (선택) 보안 강화를 위한 RLS (Row Level Security) 설정
