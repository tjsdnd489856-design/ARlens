...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _finishOnboarding() async {
    ...
    try {
      ...
      // 브랜드 코드 검증 (간이 로직: 코드가 입력되었다면 brands 테이블에 있는지 확인)
      final code = _brandCodeController.text.trim();
      if (code.isNotEmpty) {
        final brandRes = await supabase.from('brands').select().eq('id', code).maybeSingle();
        if (brandRes != null) {
          dbBrandId = brandRes['id'] as String;
          // 즉시 테마 적용
          if (mounted) {
            context.read<BrandProvider>().setBrand(Brand.fromJson(brandRes));
          }
        } else {
          // 브랜드 코드가 틀려도 진행은 시키되 경고만
          debugPrint('잘못된 브랜드 코드입니다.');
        }
      }
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isSaving
                      ...
                      : Text(_currentStep == 3 ? '시작하기' : '다음', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ),
            ),
          ],
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Text(
            '마지막으로,\n초대 코드가 있으신가요?',
            style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.black87, height: 1.3),
          ),
          const SizedBox(height: 12),
          const Text(
            'B2B 브랜드 초대 코드를 입력하시면 전용 테마와 렌즈가 제공됩니다. 없으시면 비워두셔도 좋습니다.',
